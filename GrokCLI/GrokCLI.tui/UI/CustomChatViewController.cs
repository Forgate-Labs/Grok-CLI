...
public class CustomChatViewController
{
    ...
    {
        _displayMode = mode;
        _ui.AddChatMessage($"Mode switched to {_displayMode}");
    }
    ...
    {
        _ui.AddChatMessage("Grok CLI - Agentic Mode");
        _ui.AddChatMessage($"Mode: {_displayMode} (type \"debug\" or \"normal\" to switch)");
        _ui.AddChatMessage("Commands: Ctrl+J (newline) | Ctrl+C (exit) | clear (clear terminal) | cmd <command> (run shell command)");
        _ui.AddChatMessage("Model: grok-4-1-fast-reasoning");
        _ui.AddChatMessage("");
    }
    ...
    private void RenderSearchSummary(ToolResultEvent toolEvent)
    {
        ...
        AddSummaryHeader($"Search(pattern: \"{pattern}\", path: \"{path}\")");
        ...
        if (toolEvent.Result.Success)
        {
            ...
            AddSummaryLine($"Found {matchCount} {noun}");
        }
        else
        {
    ...
    private void RenderCodeExecutionSummary(ToolResultEvent toolEvent)
    {
        ...
        AddSummaryHeader($"Python(path: \"{path}\", command: \"{snippet}\")");
    ...
    {
        var path = TryGetString(toolEvent.ArgumentsJson, "path") ?? "unknown";
        AddSummaryHeader($"Read({path})");

        if (toolEvent.Result.Success)
        {
            var lines = CountLines(toolEvent.Result.Output);
            var tokens = EstimateTokenCount(toolEvent.Result.Output);
            AddSummaryLine($"Read {lines} lines ({tokens} tokens)");
        }
        else
        {
            var message = string.IsNullOrWhiteSpace(toolEvent.Result.Error)
    ...
    private void RenderWebSearchSummary(ToolResultEvent toolEvent)
    {
        ...
        AddSummaryHeader($"WebSearch(query: \"{query}\")");
    ...
    {
        AddSummaryHeader($"{toolEvent.ToolName}()");
